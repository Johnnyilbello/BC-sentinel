...
function Fail([string]$Message) {
    Write-Host 'BC SENTINEL v0.11.0-beta.4 B4-3 BOOTSTRAP - FAIL' -ForegroundColor Red
...
try {
    ...
    if ($principal.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) { throw 'Run B4-3 from normal non-elevated PowerShell.' }
    if (-not (Test-Path -LiteralPath '.\.venv\Scripts\python.exe')) { throw '.venv not available' }

    $PatchRef = 'f7f63c4fc3f8342e25a6f1a504520581d0850e7c'
    $RepoRaw = 'https://raw.githubusercontent.com/Johnnyilbello/BC-sentinel/'

    Write-Host 'BC Sentinel v0.11.0-beta.4 - B4-3 GUIDED SAFE DATA RESCUE BOOTSTRAP' -ForegroundColor Cyan
    Write-Host 'Explicit selection + trusted scan + RR5 containment. Preview never copies data.' -ForegroundColor Yellow
    ...
    $Files = @(
        ...
        @('TEST-V011-BETA4-B43.ps1','TEST-V011-BETA4-B43.ps1'),
        @('BC_SENTINEL_V011_BETA4_B43_GUIDED_SAFE_DATA_RESCUE.md','BC_SENTINEL_V011_BETA4_B43_GUIDED_SAFE_DATA_RESCUE.md'),
    ...
    foreach ($path in $Protected) {
        ...
        if ($after -ne $Before[$path]) { throw ('B4-3 bootstrap modified protected source: ' + $path) }
    }
    Write-Host 'Protected B2 service/realtime/EDR sources unchanged after B4-3 bootstrap download.' -ForegroundColor Green
    ...
    Write-Host ('B43 BOOTSTRAP PYTHONPATH_ROOT=' + $RepoRoot) -ForegroundColor DarkGray

    try {
        Write-Host 'Launching Beta3 + B4-0..B4-3 regression, deterministic gates and live Windows data-rescue acceptance...' -ForegroundColor DarkCyan
        & powershell.exe -NoProfile -ExecutionPolicy Bypass -File '.\TEST-V011-BETA4-B43.ps1'
        if ($LASTEXITCODE -ne 0) { throw 'B4-3 acceptance failed' }
    }
    finally { $env:PYTHONPATH = $OldPythonPath }

    foreach ($path in $Protected) {
        $after = (Get-FileHash -LiteralPath $path -Algorithm SHA256).Hash.ToLowerInvariant()
        if ($after -ne $Before[$path]) { throw ('B4-3 acceptance modified protected source: ' + $path) }
    }

    Write-Host 'BC SENTINEL v0.11.0-beta.4 B4-3 BOOTSTRAP - PASS' -ForegroundColor Green
    exit 0
}
