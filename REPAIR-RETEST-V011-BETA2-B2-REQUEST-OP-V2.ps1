...
function Fail([string]$Message) {
    Write-Host 'BC SENTINEL v0.11.0-beta.2 REQUEST.OP RECOVERY V2 - FAIL' -ForegroundColor Red
...
try {
    ...
    Write-Host 'BC Sentinel v0.11.0-beta.2 - REQUEST.OP RECOVERY V2 + LIVE RETEST' -ForegroundColor Cyan
    Write-Host 'Pinned immutable recovery: reuses verified 656-test/Beta1 evidence, validates the request.op lineage, rebuilds, redeploys and reruns affected live/security/performance gates.' -ForegroundColor Yellow
    ...
    # Immutable integration snapshot containing the request.op compatibility,
    # corrected B2 contracts and frozen-runtime probe. Do not use the moving
    # branch here: Windows/HTTP caches previously served stale harness copies.
    $PinnedRef = '90008186c1d7baf46e12b2d71259137200ea5f93'
    ...
    $liveOnlyText = Get-Content -Raw -LiteralPath '.\RETEST-V011-BETA2-B2-LIVE-ONLY.ps1' -Encoding UTF8
    if (-not $liveOnlyText.Contains('v011_beta2_b2_frozen_runtime_probe.py')) {
        throw 'Pinned live-only harness integrity check failed: frozen-runtime probe reference missing'
    }
    Write-Host ('PINNED HARNESS SNAPSHOT PASS: ' + $PinnedRef) -ForegroundColor Green

    Write-Host 'Verifying fail-closed request.operation -> request.op compatibility lineage...' -ForegroundColor Cyan
    ...
    $PytestTemp = Join-Path $env:TEMP ('bc-sentinel-v011-beta2-request-op-v2-' + [guid]::NewGuid().ToString('N'))
    ...
    Write-Host 'BC SENTINEL v0.11.0-beta.2 REQUEST.OP RECOVERY V2 - PASS' -ForegroundColor Green
