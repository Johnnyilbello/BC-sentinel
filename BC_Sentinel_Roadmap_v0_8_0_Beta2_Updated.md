...
Current development line: **v0.8.0 Beta — Signed Threat Intelligence & Secure Update Channel**.
...
### v0.8.0-beta.2 — Signed Threat Packages & Publisher Envelope (current)
- separate Ed25519 trust anchors for threat content and application-release provenance; private keys are never shipped;
- canonical `bcsentinel.threat-package.v1` packages for IOC, YARA and bounded behavioral advisory metadata;
- per-component hashes, validity windows, minimum product version and sequence high-water anti-rollback;
- staging validates structure and compiles YARA on the production dependency set before activation;
- machine-HMAC authenticated activation state and transparency history;
- atomic content publication and one-step last-known-good rollback; arbitrary downgrade remains rejected;
- signed-package IOC overlay feeds existing file/network/web matching with explicit provenance and does not replace the legacy IOC feed;
- YARA hot reload uses a machine-owned active directory and preserves last compiled rules on refresh failure;
- behavior content is advisory-only in beta.1 and the schema contains no command/script/action execution surface;
- `bcsentinel.release-envelope.v1` verifies publisher Ed25519 signature plus exact file set, SHA-256 and size;
- release-envelope validation rejects tamper, missing/extra files, unsafe paths and reparse/symlink content;
- local developer BUILD→Upgrade remains compatible while the production format is prepared for future signature-required enforcement;
- standard-user GUI can validate read-only and install one signed package through one sealed UAC action (stage + activate).

### Remaining v0.8 work
- external/offline signing pipeline and key-rotation/revocation policy;
- production remote retrieval transport, resumable download and metadata freshness policy;
- make publisher signature mandatory for distributed application/service updates after signing infrastructure exists;
- wire signed behavioral content into bounded correlation policy with native acceptance;
- signed reputation package family and delta/update efficiency;
- rollback/fault-injection tests across service restart/reboot and interrupted activation;
- release transparency/provenance export and operational recovery runbook.
...
## v0.8.0-beta.2 — Trust Key Lifecycle, Signed Reputation, Recovery & Secure Retrieval

- root-signed Ed25519 content-key keysets with anti-rollback sequence;
- content-key rotation and revocation with stable key identities;
- signed domain/network reputation component, advisory-only;
- HMAC-authenticated activation recovery journal and restart reconciliation;
- HTTPS-only exact-host retrieval foundation with certificate SHA-256 pinning;
- no redirects, URL credentials, auto-stage or auto-activation;
- deterministic local protection remains independent of cloud availability.
