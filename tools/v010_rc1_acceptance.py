...
def _compatibility_matrix() -> dict:
    failures: list[dict] = []
    checked = 0
    # Canonical enterprise/brand fixtures with realistic login/support/asset vocabulary.
    ...
    # Neutral enterprise/CDN fixtures: broad benign vocabulary must remain unscored without structural risk.
    for i in range(120):
        checked += 1
        host = f"service-{i}.corp.example"
        result = assess_page_context(
            url=f"https://{host}/portal/{i}",
            ...
            form_action=f"https://{host}/submit",
            form_fields=["email"],
...
def _local_latency() -> dict:
    ...
    for i in range(loops):
        ...
        assess_page_context(
            ...
            form_action=f"https://portal-{i % 97}.enterprise.example/session",
            form_fields=["email"],
        )
...
def run(*, service_live: bool = False) -> dict:
    ...
    clone = assess_page_context(
        ...
        visible_text="Sign in to verify",
    ...
    result = {
        "product": "BC Sentinel",
        "version": APP_VERSION,
        "milestone": RC1_PROFILE,
        "profiles": {
            "beta1_deception": HEURISTIC_PROFILE,
            "beta2_response": WEB_RESPONSE_PROFILE,
            "beta3_clone_scam": CLONE_SCAM_PROFILE,
            "rc1_consolidation": RC1_PROFILE,
        },
        "compatibility": compatibility,
        "performance": {"local_page_assessment": latency, "service_ipc": None},
        "safety": {
            "heuristic_score_cap": HEURISTIC_SCORE_CAP,
            "heuristic_only_below_high": HEURISTIC_SCORE_CAP <= 49,
            "clone_detection_preserved": clone.status == "suspicious" and clone.block_recommended is False,
            "mitm_https": False,
    ...
    result["local_passed"] = bool(
        ...
        and latency["passed"]
        and all((
            result["safety"]["heuristic_only_below_high"],
            result["safety"]["clone_detection_preserved"],
            not result["safety"]["mitm_https"],
            not result["safety"]["heuristic_auto_block"],
            not result["safety"]["cloud_required"],
        ))
    )
    if service_live:
        ...
        for i in range(25):
            t0 = time.perf_counter()
            response = client.request("web_clone_scam_assess", **{
                "url": "https://microsoft-login.example/verify",
                "declared_identity": "Microsoft",
                "page_title": "Microsoft verify",
                "visible_text": "Sign in to verify",
                "form_action": "https://collector.example/post",
                "form_fields": ["password"],
                "payment_methods": [], "link_hosts": [], "redirect_chain": [],
            })
        ...
        ipc = {
            ...
            "max_p95_ms": 750.0,
            "all_ok": all(item.get("ok") for item in responses),
        }
        ipc["passed"] = bool(ipc["all_ok"] and ipc["p95_ms"] <= ipc["max_p95_ms"])
        result["performance"]["service_ipc"] = ipc
        web = (status or {}).get("web") or {}
        final_assessment = (responses[-1].get("assessment") if responses else {}) or {}
        result["service"] = {"status": status, "assessment": final_assessment}
        result["service_live_passed"] = bool(
            (status or {}).get("ok")
            and web.get("deception_profile") == HEURISTIC_PROFILE
            and web.get("response_profile") == WEB_RESPONSE_PROFILE
            and web.get("clone_scam_profile") == CLONE_SCAM_PROFILE
            and web.get("heuristic_auto_block") is False
            and web.get("mitm_https") is False
            and final_assessment.get("profile") == CLONE_SCAM_PROFILE
            and final_assessment.get("status") == "suspicious"
            and final_assessment.get("block_recommended") is False
            and ipc["passed"]
        )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="BC Sentinel v0.10.0-rc.1 consolidation acceptance")
