param(
    [switch]$ConfirmRansomwareDetectorAcceptance
)
...
function Fail([string]$Stage, [string]$Message) {
    Write-Host ("B81 FAIL STAGE=" + $Stage + " | " + $Message) -ForegroundColor Red
    Write-Host 'BC SENTINEL v0.11.0-beta.8 B8-1 RANSOMWARE-LIKE DETECTOR ACCEPTANCE - FAIL' -ForegroundColor Red
    exit 1
}

if (-not $ConfirmRansomwareDetectorAcceptance) {
    Fail 'preflight' 'Pass -ConfirmRansomwareDetectorAcceptance to run B8-1 acceptance.'
}

$repoRoot = (Resolve-Path (Join-Path $PSScriptRoot '..\..')).Path
Set-Location $repoRoot

$expectedBranch = 'feature/v011-beta8-b81-ransomware-detector'
...
Write-Host ''
Write-Host '## BC Sentinel B8-1 - Ransomware-like Detector Acceptance'
...
foreach ($frozen in @($frozenBeta6, $frozenB80, $frozenHygiene)) {
    & git cat-file -e ($frozen + '^{commit}') 2>$null
    if ($LASTEXITCODE -ne 0) { Fail 'predecessor' ("Frozen predecessor unavailable: " + $frozen) }
}
& git merge-base --is-ancestor $frozenHygiene HEAD
if ($LASTEXITCODE -ne 0) { Fail 'predecessor' 'B8-1 does not descend from accepted repository hygiene.' }
...
$tempProbe = Join-Path $repoRoot ('.b81-python-temp-' + [guid]::NewGuid().ToString('N') + '.txt')
try {
...
$pytestBase = Join-Path $base ('b81-pytest-' + [guid]::NewGuid().ToString('N'))
Write-Host ('pytest basetemp: ' + $pytestBase)

Write-Host '[1/4] Compile B8-1...'
& $py -m compileall -q sentinel/ransomware_detector.py tests/test_v011_beta8_b81_ransomware_detector.py
if ($LASTEXITCODE -ne 0) { Fail 'compile' 'B8-1 compile gate failed.' }
Write-Host 'Compile gate: PASS'
Write-Host ''

Write-Host '[2/4] Beta5 + Beta6 + Beta7 + B8-0 regression + B8-1 deterministic tests...'
$tests = @(
    Get-ChildItem -Path 'tests/test_v011_beta5_*.py','tests/test_v011_beta6_*.py','tests/test_v011_beta7_*.py','tests/test_v011_beta8_b80_coverage_baseline.py','tests/test_v011_beta8_b81_ransomware_detector.py' |
        Sort-Object FullName |
        ForEach-Object { $_.FullName }
)
try {
    & $py -m pytest -q --basetemp "$pytestBase" @tests
    if ($LASTEXITCODE -ne 0) { Fail 'pytest' 'B8-1 regression failed.' }
}
finally {
...
Write-Host 'Complete predecessor + B8-1 deterministic gate: PASS'
Write-Host ''

Write-Host '[3/4] Frozen B8-0 baseline self-check...'
...
Write-Host '[4/4] B8-1 ransomware-like detector self-check...'
& $py -m sentinel.ransomware_detector --self-check
if ($LASTEXITCODE -ne 0) { Fail 'b81' 'B8-1 detector self-check failed.' }
& $py -c "from sentinel.ransomware_detector import self_check; r=self_check(); assert r['passed']; assert r['target_scenario_id']=='B7-RANSOMWARE-001'; assert r['coverage_status']=='PARTIAL'; assert r['positive_outcome']=='DETECTED'; assert r['backup_outcome']=='REVIEW_REQUIRED'; assert r['benign_outcome']=='NO_MATCH'; assert r['stable_round_trip']; assert r['deterministic_serialization']; assert r['evidence_ids_preserved']; assert r['process_execution'] is False; assert r['file_read'] is False; assert r['file_write'] is False; assert r['file_rename'] is False; assert r['file_delete'] is False; assert r['network_io'] is False; assert r['registry_mutation'] is False; assert r['credential_access'] is False; assert r['remediation_execution'] is False; assert r['automatic_quarantine'] is False; assert r['automatic_repair'] is False; assert r['automatic_restore'] is False; assert r['general_home_execution_authorized'] is False; assert r['delete_authorized'] is False; assert r['repair_authorized'] is False; assert r['terminate_process_authorized'] is False; assert r['trust_allowlist_mutation_authorized'] is False; assert r['privileged_system_mutation_authorized'] is False; assert r['authority_granted'] is False; assert r['execution_authority_added'] is False"
if ($LASTEXITCODE -ne 0) { Fail 'b81-safety' 'B8-1 detector safety/determinism assertions failed.' }
Write-Host 'B8-1 detector provenance/determinism/safety contract: PASS'
Write-Host ''

Write-Host 'B8-1 uses normalized synthetic in-memory observations only; ransomware coverage becomes PARTIAL, never VERIFIED, and no execution/remediation authority is added.' -ForegroundColor DarkGray
Write-Host 'BC SENTINEL v0.11.0-beta.8 B8-1 RANSOMWARE-LIKE DETECTOR ACCEPTANCE - PASS' -ForegroundColor Green
