...
def verify_lineage(root: Path) -> dict[str, Any]:
    ...
    if not all(checks.values()):
        return {"profile": PROFILE, "checkpoint": "B2-final-source-lineage", "passed": False, "checks": checks}
    ...
    checks.update({
        ...
        "service_equals_deterministic_final_transform": bool(expected_core) and service_text == expected_core,
        "realtime_equals_deterministic_final_transform": bool(expected_realtime) and realtime_text == expected_realtime,
        "service_ast_valid": core_parse_ok,
        "realtime_ast_valid": realtime_parse_ok,
    ...
    return {
        "profile": PROFILE,
        "checkpoint": "B2-final-source-lineage",
        "passed": all(checks.values()),
        "checks": checks,
        "service_sha256": _sha(service_text),
        "realtime_sha256": _sha(realtime_text),
        "expected_service_sha256": _sha(expected_core) if expected_core else "",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="BC Sentinel v0.11 Beta2 strict final source-lineage verifier")
    parser.add_argument("--root", default=str(Path(__file__).resolve().parents[1]))
    parser.add_argument("--verify-only", action="store_true")
    parser.add_argument("--output", default="acceptance-v011-beta2-b2-final-lineage.json")
    ...
    try:
        ...
        result = {"profile": PROFILE, "checkpoint": "B2-final-source-lineage", "passed": False, "error": f"{type(exc).__name__}: {exc}"}
