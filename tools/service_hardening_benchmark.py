...
def run(idle_seconds: float = 3.0, ipc_requests: int = 100, storm_files: int = 200) -> dict:
    if os.name != "nt":
    ...
    return {
        "passed": bool(ipc_ok == max(1, ipc_requests) and hardening.get("ok")),
        "service": {
            "pid": pid,
        ...
        "ipc": {
            "requests": max(1, ipc_requests),
            "successful": ipc_ok,
            "elapsed_seconds": round(ipc_elapsed, 6),
            "requests_per_second": round(ipc_ok / ipc_elapsed, 2) if ipc_elapsed else 0.0,
        },
        "benign_event_storm": {
            ...
            "cpu_percent_of_one_core": round(storm_cpu / storm_elapsed * 100.0, 2) if storm_elapsed else 0.0,
        },
        "hardening": hardening,
    }
...
def main() -> int:
    ...
    result = run(max(0.2, args.idle_seconds), max(1, args.ipc_requests), max(1, args.storm_files))
