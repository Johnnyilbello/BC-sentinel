...
from tools.v011_beta2_b2_file_observation_compat import (
    ...
    transform_realtime_text as transform_observation_realtime,
)
...
from tools.v011_beta2_b2_nonexec_observation_fix_v6 import (
    MARKER as NONEXEC_MARKER,
    transform_realtime as transform_nonexec_realtime,
)
...
NONEXEC_BACKUP_REL = Path("sentinel") / "realtime.py.pre-v011-beta2-nonexec-observation-v6.bak"
...
DIAGNOSTIC_MARKER = "bc-sentinel-v011-beta2-marker-trace-v1"
QUEUE_DIAGNOSTIC_MARKER = "bc-sentinel-v011-beta2-queue-scan-trace-v4"
...
def verify_lineage(root: Path) -> dict[str, Any]:
    ...
    paths = {
        ...
        "nonexec_backup": root / NONEXEC_BACKUP_REL,
    }
    checks: dict[str, bool] = {f"{name}_exists": path.is_file() for name, path in paths.items()}
    if not all(checks.values()):
        return {"profile": PROFILE, "checkpoint": "B2-final-source-lineage-v3", "passed": False, "checks": checks}
    ...
    nonexec_pre = _read(paths["nonexec_backup"])
    realtime_pre_sha256 = _sha(realtime_pre)

    transform_error = ""
    expected_observation_core = ""
    expected_service = ""
    expected_observation_realtime = ""
    expected_realtime = ""
    try:
        ...
        expected_observation_realtime = transform_observation_realtime(realtime_pre)
        expected_realtime = transform_nonexec_realtime(expected_observation_realtime)
    ...
    checks.update({
        ...
        "nonexec_backup_equals_clean_observation_realtime": bool(expected_observation_realtime) and nonexec_pre == expected_observation_realtime,
        "realtime_equals_deterministic_v6_transform": bool(expected_realtime) and realtime_text == expected_realtime,
        ...
        "nonexec_observation_marker_present_once": realtime_text.count(NONEXEC_MARKER) == 1,
        ...
        "realtime_callback_paths_exactly_two": realtime_text.count("self.event_callback(str(") == 2,
        ...
        "diagnostic_marker_removed_from_realtime": DIAGNOSTIC_MARKER not in realtime_text,
        "queue_diagnostic_marker_removed_from_realtime": QUEUE_DIAGNOSTIC_MARKER not in realtime_text,
        "diagnostic_trace_import_removed_from_realtime": "from .b2_diagnostic_trace import" not in realtime_text,
        "diagnostic_marker_removed_from_service": DIAGNOSTIC_MARKER not in service_text,
        "diagnostic_trace_import_removed_from_service": "from .b2_diagnostic_trace import" not in service_text,
    })

    return {
        "profile": PROFILE,
        "checkpoint": "B2-final-source-lineage-v3",
        ...
        "expected_clean_observation_realtime_sha256": _sha(expected_observation_realtime) if expected_observation_realtime else "",
        "nonexec_backup_sha256": _sha(nonexec_pre),
        ...
        "nonexec_observation": {
            "static_scan_scope_widened": False,
            "scan_thread_required": False,
            "routes_to_existing_bounded_admission": True,
        },
...
def main() -> int:
    parser = argparse.ArgumentParser(description="BC Sentinel v0.11 Beta2 strict final source-lineage verifier for bounded non-executable observation V6")
    parser.add_argument("--root", default=str(Path(__file__).resolve().parents[1]))
    parser.add_argument("--verify-only", action="store_true")
    parser.add_argument("--output", default="acceptance-v011-beta2-b2-final-lineage-v3.json")
    ...
    try:
        ...
        result = {"profile": PROFILE, "checkpoint": "B2-final-source-lineage-v3", "passed": False, "error": f"{type(exc).__name__}: {exc}"}
