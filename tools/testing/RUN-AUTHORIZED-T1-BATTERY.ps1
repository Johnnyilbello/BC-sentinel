...
if ([string]$session.repository_commit -ne $commit) {
    throw ('Session commit mismatch. Session=' + [string]$session.repository_commit + ' Current=' + $commit + '. Create a new T1 session on the current commit.')
}
...
try {
    ...
    & $py -c "import importlib.util,sys; mods=('watchdog','psutil','cryptography'); sys.exit(0 if all(importlib.util.find_spec(m) is not None for m in mods) else 1)" 2>$null
...
try {
    $ErrorActionPreference = 'SilentlyContinue'
    & $py -c "import watchdog,psutil,cryptography; print('dependency-ok')" 2>$null | Out-Null
...
try {
    ...
    foreach ($case in $cases) {
        ...
        $runnerSwitch = [string]$case.runner_switch

        & powershell.exe -NoProfile -ExecutionPolicy Bypass -File $runnerPath $runnerSwitch -Output $evidencePath | Out-Null
        if ($LASTEXITCODE -ne 0) {
