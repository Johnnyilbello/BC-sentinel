...
HELPER = '''    @staticmethod
    def _replace_file_with_retry(source: Path, target: Path, *, attempts: int = 8) -> None:
        """Atomically publish a managed file while tolerating transient Windows locks.

        Defender, indexers and other readers can briefly open a just-written file or
        its destination without FILE_SHARE_DELETE, causing os.replace() to fail with
        WinError 5/32/33. Retry only those known transient Windows conditions. All
        other errors fail immediately, and the caller remains responsible for path
        containment/reparse validation before publication.
        """
        ...
        raise ThreatPackageError(f"cannot atomically publish managed threat-intelligence file{detail}")
...
def _verify(text: str) -> None:
    ...
    if "os.replace(tmp, self.active_yara_dir)" not in text:
        raise RuntimeError("Threat-package YARA directory promotion changed unexpectedly")
...
def main() -> int:
    ...
    if result["patched"]:
        print("v0.11 threat-package Windows compatibility: atomic file replace retry installed")
    else:
