...
def run(*, service_live: bool = False) -> dict:
    canonical = assess_page_context(url="https://login.microsoftonline.com/common/oauth2/authorize", declared_identity="Microsoft", page_title="Sign in to Microsoft", form_action="https://login.microsoftonline.com/login", form_fields=["email", "password"])
    ...
    scam = assess_page_context(url="https://раypal.example/verify", declared_identity="PayPal", page_title="PayPal urgent verification", visible_text="Urgent payment refund. Account suspended.", form_action="https://capture.example/post", form_fields=["password", "card_number", "cvv"], payment_methods=["gift_card"])
    benign_commerce = assess_page_context(url="https://shop.example/checkout", page_title="Checkout", visible_text="Complete payment today", form_action="https://shop.example/pay", form_fields=["card_number", "cvv"], payment_methods=["card"])
    support = assess_page_context(url="https://192.0.2.10/support", page_title="Urgent support", visible_text="Technician remote access required. Enter password.", form_fields=["password"])
    ...
    if service_live:
        ...
        live=client.request("web_clone_scam_assess", **{"url":"https://microsoft-login.example/verify","declared_identity":"Microsoft","page_title":"Microsoft verify","visible_text":"Sign in to verify","form_action":"https://collector.example/post","form_fields":["password"],"payment_methods":[],"link_hosts":[],"redirect_chain":[]})
        result["service"]={"status":status,"assessment":live}
        web=(status or {}).get("web") or {}; assessment=(live or {}).get("assessment") or {}
        result["service_live_passed"]=bool((status or {}).get("ok") and (live or {}).get("ok") and web.get("clone_scam_profile")==CLONE_SCAM_PROFILE and web.get("page_context_auto_block") is False and assessment.get("profile")==CLONE_SCAM_PROFILE and assessment.get("status")=="suspicious" and assessment.get("block_recommended") is False)
...
def main() -> int:
    ...
    parser.add_argument("--service-live",action="store_true"); parser.add_argument("--output",type=Path); args=parser.parse_args()
    result=run(service_live=args.service_live); text=json.dumps(result,ensure_ascii=False,indent=2); print(text)
