...
function Fail([string]$Message) {
    Write-Host 'BC SENTINEL v0.11.0-beta.2 B2 QUEUE/STABILIZATION DIAGNOSTIC V5 - FAIL' -ForegroundColor Red
...
try {
    ...
    $PatchRef = '03859c165bc05a427114295dcff823866dbf7844'
    $PinnedRef = '6d833d08912b64bd0d14d4f21d7838a3718c40f7'
    $RepoRaw = 'https://raw.githubusercontent.com/Johnnyilbello/BC-sentinel/'

    Write-Host 'BC Sentinel v0.11.0-beta.2 - QUEUE -> STABILIZATION TRACE V5' -ForegroundColor Cyan
    Write-Host 'Test-first launcher: validates the patcher before touching realtime.py.' -ForegroundColor Yellow
    Write-Host 'Scope: Handler/_queue_scan -> BCS-RealtimeScan -> _scan_when_stable. Marker-only diagnostics.' -ForegroundColor Yellow
    ...
    Write-Host 'Preflight: testing the instrumentation generator BEFORE modifying realtime.py...' -ForegroundColor DarkCyan
    ...
    $PreflightTemp = Join-Path $FocusedBase ('b2-queue-v5-preflight-' + [guid]::NewGuid().ToString('N'))
    New-Item -ItemType Directory -Path $PreflightTemp -Force | Out-Null
    try {
        & $Py -m pytest -q --basetemp $PreflightTemp tests/test_v011_beta2_b2_queue_scan_trace_compat_v4.py
        if ($LASTEXITCODE -ne 0) { throw 'V5 patcher preflight failed; realtime.py was not modified' }
    }
    finally {
        Remove-Item -LiteralPath $PreflightTemp -Recurse -Force -ErrorAction SilentlyContinue
    }
    Write-Host 'PATCHER PREFLIGHT PASS' -ForegroundColor Green
    ...
    Write-Host 'Running focused V5 regression tests...' -ForegroundColor DarkCyan
    $FocusedTemp = Join-Path $FocusedBase ('b2-queue-v5-' + [guid]::NewGuid().ToString('N'))
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) { throw 'Focused V5 queue/stabilization regression suite failed' }
    }
    finally {
    ...
    if ($LASTEXITCODE -ne 0) { throw 'compileall failed after V5 instrumentation' }

    Write-Host 'Building V5 diagnostic Protection Service...' -ForegroundColor Cyan
    & powershell.exe -NoProfile -ExecutionPolicy Bypass -File '.\BUILD-SERVIZIO-PROTEZIONE.ps1'
    if ($LASTEXITCODE -ne 0) { throw 'V5 diagnostic Protection Service build failed' }

    $distService = Join-Path $PSScriptRoot 'dist\BC-Sentinel-Protection\BC-Sentinel-Protection.exe'
    if (-not (Test-Path -LiteralPath $distService)) { throw 'V5 Protection Service executable missing' }
    $distHash = (Get-FileHash -LiteralPath $distService -Algorithm SHA256).Hash.ToLowerInvariant()
    Write-Host ('V5 DIAGNOSTIC BUILD SHA256=' + $distHash) -ForegroundColor Green

    Write-Host 'Deploying V5 diagnostic service through UAC Repair...' -ForegroundColor Yellow
    ...
    if ($null -eq $repairProc -or $repairProc.ExitCode -ne 0) { throw 'V5 diagnostic service deployment failed' }

    $installedService = Join-Path $env:ProgramFiles 'BC Sentinel\Protection\BC-Sentinel-Protection.exe'
    if (-not (Test-Path -LiteralPath $installedService)) { throw 'Installed V5 Protection Service missing' }
    $installedHash = (Get-FileHash -LiteralPath $installedService -Algorithm SHA256).Hash.ToLowerInvariant()
    if ($installedHash -ne $distHash) { throw 'Installed V5 service does not match freshly built dist' }
    Write-Host ('V5 BINARY PROVENANCE PASS: SHA256=' + $installedHash) -ForegroundColor Green
    ...
    & $Py -m tools.v011_beta2_b2_service_startup_probe `
        ...
        --output acceptance-v011-beta2-b2-v5-service-startup.json
    if ($LASTEXITCODE -ne 0) { throw 'V5 service named pipe was not ready within 20 seconds' }

    Write-Host 'Running V5 multi-root marker trace...' -ForegroundColor Cyan
    & $Py -m tools.v011_beta2_b2_trace_probe `
        --poll-seconds 12 `
        --output acceptance-v011-beta2-b2-queue-stabilization-v5.json
    if ($LASTEXITCODE -ne 0) { throw 'V5 marker trace probe execution failed' }

    $probePath = Join-Path $PSScriptRoot 'acceptance-v011-beta2-b2-queue-stabilization-v5.json'
    if (-not (Test-Path -LiteralPath $probePath)) { throw 'V5 marker trace output missing' }
    $probe = Get-Content -Raw -LiteralPath $probePath -Encoding UTF8 | ConvertFrom-Json

    Write-Host '--- V5 QUEUE/STABILIZATION ROOT-CAUSE SUMMARY ---' -ForegroundColor Cyan
    ...
    Write-Host 'BC SENTINEL v0.11.0-beta.2 B2 QUEUE/STABILIZATION DIAGNOSTIC V5 - COMPLETE' -ForegroundColor Green
    exit 0
}
catch {
