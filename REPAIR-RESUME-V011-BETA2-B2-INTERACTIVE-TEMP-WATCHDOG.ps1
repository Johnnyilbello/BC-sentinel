...
function Fail([string]$Message) {
    Write-Host 'BC SENTINEL v0.11.0-beta.2 B2 SERVICE-SETTINGS OWNERSHIP REPAIR - FAIL' -ForegroundColor Red
...
try {
    ...
    Write-Host 'BC Sentinel v0.11.0-beta.2 - B2 SERVICE-SETTINGS OWNERSHIP REPAIR' -ForegroundColor Cyan
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) { throw 'Focused service-settings/profile/EDR/watchdog/low-CPU/service-performance regression failed' }
    }
    finally {
    ...
    Write-Host ('CORRECTED SERVICE-SETTINGS BUILD: SHA256=' + $distHash) -ForegroundColor Green
    ...
    Write-Host ('DEPLOYED SERVICE-SETTINGS BUILD: SHA256=' + $installedHash) -ForegroundColor Green
