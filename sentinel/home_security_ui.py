...
"""Stitch source-of-truth shell for BC Sentinel B6-2.

The dashboard implementation remains isolated in ``home_security_ui_impl`` as a
presentation component; this module owns the real application shell, secondary
Stitch surfaces, navigation and responsive contract. Security state and actions
remain delegated to the accepted model/engine interfaces.
"""
...
from PySide6.QtWidgets import QApplication, QBoxLayout, QFrame, QLabel, QScrollArea, QSizePolicy, QStackedWidget, QVBoxLayout, QWidget

from sentinel import home_security_model as model
from sentinel import home_security_ui_impl as _impl
from sentinel.ui_design_system import BREAKPOINTS, COLORS, MOTION, NAV_ITEMS as DESIGN_NAV_ITEMS, SPACING
...
COLOR_TOKENS = {
    "canvas": COLORS["canvas"], "sidebar": COLORS["sidebar"], "surface_lowest": COLORS["surface_lowest"],
    "surface_low": COLORS["surface_low"], "surface_1": COLORS["surface"], "surface_2": COLORS["surface_high"],
    "surface_3": COLORS["surface_highest"], "border": COLORS["border"], "border_soft": COLORS["border_soft"],
    "text_primary": COLORS["text"], "text_secondary": COLORS["text_secondary"], "text_muted": COLORS["text_muted"],
    "accent": COLORS["accent"], "accent_bright": COLORS["accent_bright"], "success": COLORS["accent_bright"],
    "warning": COLORS["warning"], "critical": COLORS["danger"], "cool": COLORS["info"],
}
...
class _PageScroll(QScrollArea):
    """Vertical-only wrapper for secondary pages; host width follows viewport."""
    def __init__(self, page: QWidget, parent: QWidget | None = None) -> None:
        ...
        host = QWidget(); host.setObjectName("SecondaryPageHost"); host.setMinimumWidth(0); host.setSizePolicy(QSizePolicy.Policy.Ignored, QSizePolicy.Policy.Preferred)
        lay = QVBoxLayout(host); lay.setContentsMargins(24, 24, 24, 32); lay.setSpacing(0)
        page.setMinimumWidth(0); page.setSizePolicy(QSizePolicy.Policy.Ignored, QSizePolicy.Policy.Preferred); lay.addWidget(page); lay.addStretch(1)
        self.setWidget(host); self.page = page; self.host = host
    def sync_width(self) -> None:
        width = max(0, self.viewport().width())
        if width:
            self.host.setFixedWidth(width)
        self.host.updateGeometry(); self.page.updateGeometry()


class SecurityOverviewWindow(_impl.SecurityOverviewWindow):
    """Complete six-surface Sentinel Elite application shell."""
    ...
    def _install_complete_shell(self) -> None:
        ...
        self.secondary_scrolls = [self.scan_scroll, self.quarantine_scroll, self.history_scroll, self.protection_scroll, self.settings_scroll]

        self.stack = QStackedWidget(); self.stack.setObjectName("PageStack"); self.stack.setMinimumWidth(0); self.stack.setSizePolicy(QSizePolicy.Policy.Ignored, QSizePolicy.Policy.Expanding)
        self.stack.addWidget(self.page_scroll)
        for scroll in self.secondary_scrolls: self.stack.addWidget(scroll)
        main_layout.addWidget(self.stack, 1)

        for index, name in enumerate(PAGE_ORDER):
            button = self.nav_buttons[name]
            button.setEnabled(True); button.setCheckable(True); button.setChecked(index == 0)
        ...
        self._brand_labels = [w for w in (brand_name, brand_edition) if w is not None]
        self._footer_buttons = self.sidebar.findChildren(QWidget, "SidebarFooterItem")
        self._footer_texts = [b.text() for b in self._footer_buttons]
        self._sync_page_host_width()

    def _navigate(self, page: str) -> None:
        if page not in PAGE_ORDER: return
    ...
    def _apply_responsive_layout(self, force: bool = False) -> None:
        ...
        for name, button in self.nav_buttons.items(): button.setText("" if mobile else name)
        ...
        for scroll in self.secondary_scrolls:
            if scroll.host.layout(): scroll.host.layout().setContentsMargins(margin, margin, margin, 32)
        QTimer.singleShot(0, self._sync_all_scroll_widths)
