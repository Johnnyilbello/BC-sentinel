...
def test_v010_beta1_version_and_release_artifacts():
    ...
    package_version = APP_VERSION.replace("-alpha.", "a").replace("-beta.", "b").replace("-rc.", "rc")
    assert f'version = "{package_version}"' in (ROOT / "pyproject.toml").read_text(encoding="utf-8")
