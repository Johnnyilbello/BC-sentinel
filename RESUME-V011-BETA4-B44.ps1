...
function Fail([string]$Message) {
    Write-Host 'BC SENTINEL v0.11.0-beta.4 B4-4 BOOTSTRAP - FAIL' -ForegroundColor Red
...
try {
    ...
    if ($principal.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) { throw 'Run B4-4 from normal non-elevated PowerShell.' }
    if (-not (Test-Path -LiteralPath '.\.venv\Scripts\python.exe')) { throw '.venv not available' }

    $PatchRef = '872efd59370e6fe2334552650aab4f76db443466'
    $RepoRaw = 'https://raw.githubusercontent.com/Johnnyilbello/BC-sentinel/'

    Write-Host 'BC Sentinel v0.11.0-beta.4 - B4-4 INTEGRATED CERTIFICATION + SESSION SUMMARY BOOTSTRAP' -ForegroundColor Cyan
    Write-Host 'Validate Beta4 session continuity first; preserve RR6 outcome exactly; no new mutation authority.' -ForegroundColor Yellow
    ...
    $Files = @(
        ...
        @('TEST-V011-BETA4-B44.ps1','TEST-V011-BETA4-B44.ps1'),
        @('BC_SENTINEL_V011_BETA4_B44_INTEGRATED_CERTIFICATION_SESSION_SUMMARY.md','BC_SENTINEL_V011_BETA4_B44_INTEGRATED_CERTIFICATION_SESSION_SUMMARY.md'),
    ...
    foreach ($path in $Protected) {
        ...
        if ($after -ne $Before[$path]) { throw ('B4-4 bootstrap modified protected source: ' + $path) }
    }
    Write-Host 'Protected B2 service/realtime/EDR sources unchanged after B4-4 bootstrap download.' -ForegroundColor Green
    ...
    Write-Host ('B44 BOOTSTRAP PYTHONPATH_ROOT=' + $RepoRoot) -ForegroundColor DarkGray

    try {
        Write-Host 'Launching Beta3 + B4-0..B4-4 regression, deterministic gates and live Windows integrated-certification acceptance...' -ForegroundColor DarkCyan
        & powershell.exe -NoProfile -ExecutionPolicy Bypass -File '.\TEST-V011-BETA4-B44.ps1'
        if ($LASTEXITCODE -ne 0) { throw 'B4-4 acceptance failed' }
    }
    finally { $env:PYTHONPATH = $OldPythonPath }

    foreach ($path in $Protected) {
        $after = (Get-FileHash -LiteralPath $path -Algorithm SHA256).Hash.ToLowerInvariant()
        if ($after -ne $Before[$path]) { throw ('B4-4 acceptance modified protected source: ' + $path) }
    }

    Write-Host 'BC SENTINEL v0.11.0-beta.4 B4-4 BOOTSTRAP - PASS' -ForegroundColor Green
    exit 0
}
