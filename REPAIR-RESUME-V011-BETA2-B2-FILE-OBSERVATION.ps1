...
try {
    ...
    $PatchRef = '353cd5b19e26033aa85c011db5fba90600e092dc'
    ...
    Write-Host 'Adds non-destructive filesystem observation after watchdog stabilization and before static scan.' -ForegroundColor Yellow
    ...
    $patchFiles = @(
        ...
        @('tools\v011_beta2_b2_final_lineage.py','tools/v011_beta2_b2_final_lineage.py'),
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) { throw 'Focused final B2 file-observation regression failed' }
    }
    finally {
    ...
    if ($LASTEXITCODE -ne 0) { throw 'compileall failed after file-observation patch' }

    & $Py -m tools.v011_beta2_b2_final_lineage --verify-only --output acceptance-v011-beta2-b2-final-lineage-prebuild.json
    if ($LASTEXITCODE -ne 0) { throw 'Strict final source-lineage verification failed before build' }

    Write-Host 'Building final Protection Service with stabilized watchdog observation...' -ForegroundColor Cyan
    & powershell.exe -NoProfile -ExecutionPolicy Bypass -File '.\BUILD-SERVIZIO-PROTEZIONE.ps1'
    if ($LASTEXITCODE -ne 0) { throw 'Fresh Protection Service build failed after file-observation patch' }
    ...
    Write-Host ('FINAL FILE-OBSERVATION BUILD: SHA256=' + $distHash) -ForegroundColor Green
    ...
    # Supply the strict deterministic final verifier under that name.
    Download-RequiredFile `
        '.\tools\v011_beta2_b2_request_op_compat.py' `
        ($RepoRaw + $PatchRef + '/tools/v011_beta2_b2_final_lineage.py')
    ...
    if ($LASTEXITCODE -ne 0) { throw 'Strict final source-lineage verification failed before live gate' }
    ...
    Write-Host 'BC SENTINEL v0.11.0-beta.2 B2 FILE-OBSERVATION REPAIR - PASS' -ForegroundColor Green
    exit 0
}
catch {
