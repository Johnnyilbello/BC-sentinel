...
function Fail([string]$Message) {
    Write-Host 'BC SENTINEL v0.11.0-beta.4 B4-2 BOOTSTRAP - FAIL' -ForegroundColor Red
...
try {
    ...
    if ($principal.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) { throw 'Run B4-2 from normal non-elevated PowerShell.' }
    if (-not (Test-Path -LiteralPath '.\.venv\Scripts\python.exe')) { throw '.venv not available' }

    $PatchRef = 'faec3ee40b843407272c1566189247696550675b'
    $RepoRaw = 'https://raw.githubusercontent.com/Johnnyilbello/BC-sentinel/'

    Write-Host 'BC Sentinel v0.11.0-beta.4 - B4-2 GUIDED REPAIR HANDOFF BOOTSTRAP' -ForegroundColor Cyan
    Write-Host 'Trusted evidence -> approved operations -> RR4B plan/token handoff. Console adds no new mutation authority.' -ForegroundColor Yellow
    ...
    $Files = @(
        ...
        @('TEST-V011-BETA4-B42.ps1','TEST-V011-BETA4-B42.ps1'),
        @('BC_SENTINEL_V011_BETA4_B42_GUIDED_REPAIR_HANDOFF.md','BC_SENTINEL_V011_BETA4_B42_GUIDED_REPAIR_HANDOFF.md'),
    ...
    foreach ($path in $Protected) {
        ...
        if ($after -ne $Before[$path]) { throw ('B4-2 bootstrap modified protected source: ' + $path) }
    }
    Write-Host 'Protected B2 service/realtime/EDR sources unchanged after B4-2 bootstrap download.' -ForegroundColor Green
    ...
    Write-Host ('B42 BOOTSTRAP PYTHONPATH_ROOT=' + $RepoRoot) -ForegroundColor DarkGray

    try {
        Write-Host 'Launching Beta3 + B4-0..B4-2 regression, deterministic gates and live Windows handoff acceptance...' -ForegroundColor DarkCyan
        & powershell.exe -NoProfile -ExecutionPolicy Bypass -File '.\TEST-V011-BETA4-B42.ps1'
        if ($LASTEXITCODE -ne 0) { throw 'B4-2 acceptance failed' }
    }
    finally {
    ...
    foreach ($path in $Protected) {
        $after = (Get-FileHash -LiteralPath $path -Algorithm SHA256).Hash.ToLowerInvariant()
        if ($after -ne $Before[$path]) { throw ('B4-2 acceptance modified protected source: ' + $path) }
    }

    Write-Host 'BC SENTINEL v0.11.0-beta.4 B4-2 BOOTSTRAP - PASS' -ForegroundColor Green
    exit 0
}
