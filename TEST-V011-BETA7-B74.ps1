param(
    [switch]$ConfirmAttackChainAcceptance
)
...
function Fail([string]$Stage, [string]$Message) {
    Write-Host ("B74 FAIL STAGE=" + $Stage + " | " + $Message) -ForegroundColor Red
    Write-Host "BC SENTINEL v0.11.0-beta.7 B7-4 ATTACK-CHAIN ACCEPTANCE HARNESS - FAIL" -ForegroundColor Red
    exit 1
}

if (-not $ConfirmAttackChainAcceptance) {
    Fail "preflight" "B7-4 requires -ConfirmAttackChainAcceptance."
}
...
$FrozenB73 = "eef5c40edc853ff3f13cb521ce980dcaaea75fd0"

Write-Host ""
Write-Host "## BC Sentinel B7-4 - Attack-Chain Acceptance Harness"
Write-Host ("Branch: " + $Branch)
Write-Host ("Commit: " + $Commit)
Write-Host ("Frozen B7-3 predecessor: " + $FrozenB73)
Write-Host ""

foreach ($Frozen in @($FrozenBeta6, $FrozenB73)) {
...
foreach ($GitPath in $FrozenB73Files) {
    & git diff --quiet $FrozenB73 HEAD -- $GitPath
    if ($LASTEXITCODE -ne 0) {
        Fail "predecessor-freeze" ("Accepted B7-0/B7-1/B7-2/B7-3 foundation changed: " + $GitPath)
    }
}
Write-Host "Accepted B7-0/B7-1/B7-2/B7-3 foundations unchanged: PASS"

$TempProbe = Join-Path $RepoRoot (".b74-python-temp-" + [guid]::NewGuid().ToString("N") + ".txt")
try {
...
$PytestBase = Join-Path $Base ("b74-pytest-" + [guid]::NewGuid().ToString("N"))
Write-Host ("pytest basetemp: " + $PytestBase)

Write-Host "[1/7] Compile B7-4..."
...
if ($LASTEXITCODE -ne 0) { Fail "compile" "B7-4 compile gate failed." }
Write-Host "Compile gate: PASS"
Write-Host ""

Write-Host "[2/7] Beta5 + Beta6 + B7-0/B7-1/B7-2/B7-3 predecessor regression + B7-4 deterministic tests..."
$Tests = @(
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { Fail "pytest" "Predecessor/B7-4 deterministic gate failed." }
}
finally {
...
Write-Host "Predecessor + B7-4 deterministic gate: PASS"
Write-Host ""

Write-Host "[3/7] B7-0 coverage ledger self-check..."
...
Write-Host "[4/7] B7-1 Security Graph self-check..."
...
Write-Host "[5/7] B7-2 Incident Correlation self-check..."
...
Write-Host "[6/7] B7-3 Confidence Gate self-check..."
...
Write-Host "[7/7] B7-4 controlled Attack-Chain Harness self-check..."
...
Write-Host "Attack-Chain determinism + harmlessness + safety contract: PASS"
Write-Host ""
Write-Host "B7-4 uses synthetic in-memory fixtures only; advisory interruption eligibility never grants execution authority." -ForegroundColor DarkGray
Write-Host "BC SENTINEL v0.11.0-beta.7 B7-4 ATTACK-CHAIN ACCEPTANCE HARNESS - PASS" -ForegroundColor Green
