...
function Fail([string]$Message) {
    Write-Host 'BC SENTINEL v0.11.0-beta.4 B4-1 BOOTSTRAP - FAIL' -ForegroundColor Red
...
try {
    ...
    if ($principal.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) { throw 'Run B4-1 from normal non-elevated PowerShell.' }
    if (-not (Test-Path -LiteralPath '.\.venv\Scripts\python.exe')) { throw '.venv not available' }

    $PatchRef = '81020a9175088644b1d7e8a17cb66b4a40acd5e4'
    $RepoRaw = 'https://raw.githubusercontent.com/Johnnyilbello/BC-sentinel/'

    Write-Host 'BC Sentinel v0.11.0-beta.4 - B4-1 EVIDENCE INVENTORY + GUIDED SCAN BOOTSTRAP' -ForegroundColor Cyan
    Write-Host 'Read-only evidence inventory and explicit RR3 scanning. No automatic repair/quarantine authority.' -ForegroundColor Yellow
    ...
    $Files = @(
        ...
        @('TEST-V011-BETA4-B41.ps1','TEST-V011-BETA4-B41.ps1'),
        @('BC_SENTINEL_V011_BETA4_B40_RESCUE_CONSOLE_FOUNDATION.md','BC_SENTINEL_V011_BETA4_B40_RESCUE_CONSOLE_FOUNDATION.md'),
        @('BC_SENTINEL_V011_BETA4_B41_EVIDENCE_INVENTORY_GUIDED_SCAN.md','BC_SENTINEL_V011_BETA4_B41_EVIDENCE_INVENTORY_GUIDED_SCAN.md'),
    ...
    foreach ($path in $Protected) {
        ...
        if ($after -ne $Before[$path]) { throw ('B4-1 bootstrap modified protected source: ' + $path) }
    }
    Write-Host 'Protected B2 service/realtime/EDR sources unchanged after B4-1 bootstrap download.' -ForegroundColor Green
    ...
    Write-Host ('B41 BOOTSTRAP PYTHONPATH_ROOT=' + $RepoRoot) -ForegroundColor DarkGray

    try {
        Write-Host 'Launching Beta3 + B4-0 regression, B4-1 deterministic and live Windows guided-scan acceptance...' -ForegroundColor DarkCyan
        & powershell.exe -NoProfile -ExecutionPolicy Bypass -File '.\TEST-V011-BETA4-B41.ps1'
        if ($LASTEXITCODE -ne 0) { throw 'B4-1 acceptance failed' }
    }
    finally {
    ...
    foreach ($path in $Protected) {
        $after = (Get-FileHash -LiteralPath $path -Algorithm SHA256).Hash.ToLowerInvariant()
        if ($after -ne $Before[$path]) { throw ('B4-1 acceptance modified protected source: ' + $path) }
    }

    Write-Host 'BC SENTINEL v0.11.0-beta.4 B4-1 BOOTSTRAP - PASS' -ForegroundColor Green
    exit 0
}
