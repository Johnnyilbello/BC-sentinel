...
Write-Host "BC Sentinel v0.10.0-rc.1 - ADMIN PHASE (no reboot)" -ForegroundColor Cyan

# Native/security + Beta1/Beta2/RC1 targeted regressions.
& $Py -m pytest -q `
...
# the real machine upgrade when the installed version is older than RC1.
$upgradePlanPath = Join-Path $PSScriptRoot "acceptance-v010-rc1-upgrade-plan.json"
...
# REPAIR gate: must always be real and same-version after upgrade/current RC1.
$repairPlanPath = Join-Path $PSScriptRoot "acceptance-v010-rc1-repair-plan.json"
...
& $Py -m tools.v010_web_deception_acceptance --service-live --output acceptance-v010-rc1-deception-live.json
if ($LASTEXITCODE -ne 0) { throw "Beta1 deception live fallita" }
& $Py -m tools.v010_web_response_acceptance --service-live --output acceptance-v010-rc1-response-live.json
if ($LASTEXITCODE -ne 0) { throw "Beta2 reversible response live fallita" }
& $Py -m tools.v010_clone_scam_acceptance --service-live --output acceptance-v010-rc1-clone-scam-live.json
if ($LASTEXITCODE -ne 0) { throw "RC1 clone/scam live fallita" }
& $Py -m tools.web_threat_response_acceptance --service-live --output acceptance-v010-rc1-legacy-response-live.json
if ($LASTEXITCODE -ne 0) { throw "Legacy web response live fallita" }
& $Py -m tools.v010_rc1_acceptance --service-live --output acceptance-v010-rc1-consolidation-live.json
if ($LASTEXITCODE -ne 0) { throw "RC1 consolidation live fallita" }
& $Py -m tools.windows_acceptance --benchmark-files 5000 --realtime-seconds 3 --service-live --output acceptance-v010-rc1-windows-live.json
if ($LASTEXITCODE -ne 0) { throw "Windows acceptance live fallita" }
& $Py -m tools.service_hardening_benchmark --idle-seconds 5 --ipc-requests 200 --storm-files 500 --output benchmark-v010-rc1-service.json
