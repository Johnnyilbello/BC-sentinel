...
### v0.7.2-beta.3 — Domain Trust, Provenance & Browser Context (native Windows accepted)
...
### v0.7.2-beta.4 — Browser Download Protection & Web Incident Chain (native Windows accepted via v0.8 regression)
...
- no new protection feature class; freezes the v0.8 architecture for release validation;
- dedicated RC acceptance requires Beta1→Beta3 threat-channel regressions;
- release/version ordering, documentation and private-key marker audit;
- remote retrieval remains no-auto-stage/no-auto-activate and cloud-independent;
- native Windows aggregate acceptance remains mandatory before v0.8 freeze.

### v0.8.0-beta.3 — Signed Remote Threat Index, Controlled Retrieval & Revocation Operations (native Windows accepted)
- separate pinned Ed25519 trust anchor for the signed remote threat index;
- `bcsentinel.threat-index.v1` metadata covers package id/sequence/version, SHA-256, exact byte size, HTTPS URL, signing key, components and minimum product version;
- independent HMAC-authenticated index high-water anti-rollback;
- candidate selection excludes revoked/unavailable content-signing keys and non-newer/incompatible packages;
- HTTPS-pinned retrieval verifies signed-index envelope, exact package envelope hash/size and the inner threat-package signature/key lifecycle;
- SHA-256 content-addressed bounded ThreatCache with atomic writes and reparse protection;
- authenticated bounded retrieval scheduler with minimum one-hour cadence and bounded retry backoff;
- retrieval may discover/cache a candidate but remains `auto_stage=false` and `auto_activate=false`;
- active content signed by a subsequently revoked key enters `critical_replacement_required` while remaining readable until a verified replacement is explicitly activated;
- no automatic destructive action, no fail-open behavior and no loss of offline deterministic protection;
- activation fault-injection matrix covers package promote, journal, IOC, reputation, YARA, behavior, state commit and journal clear, requiring complete old-or-new state only.

### v0.8.0-beta.2 — Trust Key Lifecycle, Signed Reputation, Recovery & Secure Retrieval Foundation (native Windows accepted)
- root-pinned Ed25519 threat-content keysets with key rotation, explicit revocation and keyset high-water anti-rollback;
- revoked content keys cannot validate new packages; legacy Beta1 key compatibility remains controlled by the root-signed keyset;
- signed domain/network reputation with confidence/expiry/provenance remains advisory-only and cannot directly create firewall/quarantine actions;
- HMAC-authenticated activation recovery journal restores previous committed content after interrupted publication or confirms a completed post-state-write activation;
- HTTPS-only retrieval foundation uses exact-host allowlist, TCP/443, peer-certificate SHA-256 pinning, bounded response size and optional expected content hash;
- redirects and URL credentials are rejected; retrieval remains separate from stage/activation and cloud is not required;
- native Windows aggregate acceptance passed with zero critical failures.

### v0.8.0-beta.1 — Signed Threat Packages & Secure Update Channel Foundation
- separate Ed25519 trust anchors for threat content and application-release provenance; private keys are never shipped;
- canonical `bcsentinel.threat-package.v1` packages for IOC, YARA, bounded behavioral advisory metadata and later signed reputation;
- per-component hashes, validity windows, minimum product version and sequence high-water anti-rollback;
- staging validates structure and compiles YARA before activation on the production Windows dependency set;
- machine-HMAC authenticated activation state and transparency history;
- atomic content publication and one-step last-known-good rollback while arbitrary downgrade remains rejected;
- signed-package IOC overlay feeds existing file/network/web matching with explicit provenance;
- YARA hot reload uses a machine-owned active directory and behavior content remains advisory-only;
- `bcsentinel.release-envelope.v1` verifies publisher Ed25519 signature, exact file set, SHA-256 and size;
- local developer BUILD→Upgrade remains compatible while production release signing infrastructure is prepared.

### v0.8 freeze status
- v0.8.0-rc.1 native Windows acceptance completed with all critical gates passing;
- signed threat packages, key lifecycle, remote signed index, controlled retrieval, rollback/recovery and release-hardening invariants are frozen as the baseline for v0.9;
- external production key-custody and distributed publisher-signing infrastructure remain operationalization work, not a prerequisite for local deterministic protection.

## v0.9 — Antispyware & Advanced Antimalware

### v0.9.0-beta.1 — Antispyware & Advanced Antimalware Foundation (native feature gate passed; aggregate freeze evidence pending)
- read-only Run/RunOnce, Startup folder and Scheduled Task discovery;
- automatic-service persistence inventory;
- WMI permanent command-consumer inventory;
- Chrome/Edge/Firefox policy discovery;
- proxy and DNS configuration inventory with baseline-change evidence;
- stable `BCP-*` antispyware findings with persistence provenance;
- Authenticode, missing-target, user-writable/temp-path, interpreter and encoded-command context;
- persistence-only heuristics capped below HIGH;
- multi-signal correlation with qualified file/network evidence into the existing Incident Engine;
- no automatic registry/task/service/WMI/browser/network deletion in Beta1;
- new critical Windows gates `antispyware-v090-beta1-foundation` and `antispyware-v090-beta1-live`.

### v0.9.0-beta.2 — Reversible Persistence Remediation & PUP/Adware Response (current)
- HMAC-SHA256 authenticated remediation plans linked to stable `BCP-*` findings;
- exact snapshot + pre-mutation anti-race verification;
- explicit disable/restore for Run/RunOnce and selected browser-policy Registry values;
- Startup-file vault move/restore with SHA-256 verification and reparse-point guards;
- Scheduled Task disable/enable and automatic-service start-type auto→manual→auto restoration;
- no service process termination by antispyware remediation;
- loaded interactive-user `HKEY_USERS\S-1-5-21-*` Run/RunOnce inventory from LocalSystem service context;
- non-standard forced-extension and bounded PUP/adware classification, always below destructive-malware semantics by itself;
- WMI/DNS/proxy remain review-only until stronger reversible semantics exist;
- apply/restore are privileged operations and compatible with the one-action UAC broker;
- critical Windows gates `antispyware-v090-beta2-foundation` and `antispyware-v090-beta2-live`.

### v0.9.0-beta.3 — Advanced Antimalware & Fileless Correlation
- PowerShell/script/LOLBin behavior correlation;
- stronger credential-stealer indicators;
- fileless/process-chain evidence with conservative false-positive gates;
- advanced ransomware recovery/journal integration where safety gates are satisfied.

### v0.9.0-rc.1 — Antimalware Consolidation & Native Hardening
- complete v0.9 regression freeze and native Windows acceptance before v0.10.
