...
function Fail([string]$Message) {
    Write-Host 'BC SENTINEL v0.11.0-beta.4 B4-5 BOOTSTRAP - FAIL' -ForegroundColor Red
...
try {
    ...
    if ($principal.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) { throw 'Run B4-5 from normal non-elevated PowerShell.' }
    if (-not (Test-Path -LiteralPath '.\.venv\Scripts\python.exe')) { throw '.venv not available' }

    $PatchRef = '1daa639e409ab86e77c32b07c11e15dba67b3d3d'
    $RepoRaw = 'https://raw.githubusercontent.com/Johnnyilbello/BC-sentinel/'

    Write-Host 'BC Sentinel v0.11.0-beta.4 - B4-5 PORTABLE RESCUE CONSOLE BOOTSTRAP' -ForegroundColor Cyan
    Write-Host 'Portable onedir build + built-artifact acceptance. No installer/service/driver and no repair-execute console path.' -ForegroundColor Yellow
    ...
    $Files = @(
        ...
        @('packaging\rescue_console_portable_entry.py','packaging/rescue_console_portable_entry.py'),
        ...
        @('BUILD-RESCUE-CONSOLE-PORTABLE.ps1','BUILD-RESCUE-CONSOLE-PORTABLE.ps1'),
        @('TEST-V011-BETA4-B45.ps1','TEST-V011-BETA4-B45.ps1'),
        @('BC_SENTINEL_V011_BETA4_B45_PORTABLE_RESCUE_CONSOLE.md','BC_SENTINEL_V011_BETA4_B45_PORTABLE_RESCUE_CONSOLE.md'),
    ...
    foreach ($path in $Protected) {
        ...
        if ($after -ne $Before[$path]) { throw ('B4-5 bootstrap modified protected source: ' + $path) }
    }
    Write-Host 'Protected B2 service/realtime/EDR sources unchanged after B4-5 bootstrap download.' -ForegroundColor Green
    ...
    Write-Host ('B45 BOOTSTRAP PYTHONPATH_ROOT=' + $RepoRoot) -ForegroundColor DarkGray

    try {
        Write-Host 'Launching Beta3 + complete Beta4 regression, hardened build and built-artifact Windows acceptance...' -ForegroundColor DarkCyan
        & powershell.exe -NoProfile -ExecutionPolicy Bypass -File '.\TEST-V011-BETA4-B45.ps1'
        if ($LASTEXITCODE -ne 0) { throw 'B4-5 acceptance failed' }
    }
    finally { $env:PYTHONPATH = $OldPythonPath }

    foreach ($path in $Protected) {
        $after = (Get-FileHash -LiteralPath $path -Algorithm SHA256).Hash.ToLowerInvariant()
        if ($after -ne $Before[$path]) { throw ('B4-5 acceptance modified protected source: ' + $path) }
    }

    Write-Host 'BC SENTINEL v0.11.0-beta.4 B4-5 BOOTSTRAP - PASS' -ForegroundColor Green
    exit 0
}
